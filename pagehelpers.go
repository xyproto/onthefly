...
func PublishFile(m *martini.ClassicMartini, url, filename string) {
	m.Get(url, File(filename))
}

// Takes a filename and offers that file at the root url
func PublishRootFile(m *martini.ClassicMartini, filename string) {
	m.Get("/"+filename, File(filename))
}

// Expose the HTML and CSS generated by a page building function to the two given urls
func PublishPage(m *martini.ClassicMartini, htmlurl, cssurl string, buildfunction func(string) *Page) {
	page := buildfunction(cssurl)
	m.Get(htmlurl, GenerateHTML(page))
	m.Get(cssurl, GenerateCSS(page))
}
...
func Publish(m *martini.ClassicMartini, url, filename string, cache bool) {
	if cache {
		m.Get(url, CacheWrapper(url, File(filename)))
	} else {
		m.Get(url, File(filename))
	}
}
